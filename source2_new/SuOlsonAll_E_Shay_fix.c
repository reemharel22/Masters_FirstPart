...
//@@@CHANGED TO 2*NN +1
double L[NN],U[NN],mainD[NN],E[X+1][N],EF[X+1],D[X],solve[NN],Weff[X+1][N];
double x0 = 0.5;
double t0 = 10.0;
double A[X+1];
double eps = 1.0;
...
void PredictorCorrectorSolution(int times,int i, void(*f)(),void(*BuildLUD)(),void(*ApplyTS)(),void(*copySolve)()) {
    ...
    for (int k = 0; k < NN; k++) {
        if (k % 2 == 1) {
            double weff = calculateWeff(k/2 , currentTimeStep - 1);
            r[k] = solve[k] * calculateMu2(weff)*calculateA(weff);
        } else {
            r[k] = solve[k];
        }
    }
    solveTriagonal(NN,&solve,L,U,mainD,r); // solve
   //\ tridag(L,mainD,U,r,solve,NN);
...
void constructLUDP1(double (*L)[NN],double (*U)[NN],double (*mainD)[NN]) {
    ...
    for (i = 0; i < NN; i++) {
      if (i % 2 == 0) {//E
          ...
          double aa = calculateA(calculateWeff(j,currentTimeStep - 1));
          double bb = calculateB(calculateWeff(j,currentTimeStep - 1));
          (*mainD)[i] = mu*aa + deltaT*c*opacity*bb*mu;
          //(*mainD)[i] = 1.0 + deltaT*opacity*c;
          j++;
      }
    }
...
double calculateWeff(int space,int time1) {
    ...
    if (!constOpacity && P1 && space <= X-1) {
        ...
        double op1 = getOpacity(space,time1);
...
void applyBC() {
  if (!constOpacity) {
      double mu = calculateMu2(calculateWeffNonAvg(0,currentTimeStep - 1));
