...
//@@@CHANGED TO 2*NN +1
double L[NN],U[NN],mainD[NN],E[X][N],EF[X],D[X],solve[NN],Weff[X+1][N];
double waveFront[X];
double TH[2][759];
...
double getOpacity(int space,int time1) {
    if (constOpacity == 1) {
        ...
        double xx = s_g * pow(getT(space,time1), alpha) * pow(rho, -s_lambda);
        double bbbb = rho / xx; 

       // double tmp = (1.0 * pow(rho, s_lambda + 1.0)/(s_g * pow(getT(space,time1), alpha)));
       // if (tmp != tmp)
...
void setUpInitialCondition() {
    ...
    for (i = 0; i < X; i++) {
       solve[i] = getOpacity(i, 0) * c * deltaT * T[i][0];
    }applyBC(0);
    for ( i = 0; i < X; i++) {
