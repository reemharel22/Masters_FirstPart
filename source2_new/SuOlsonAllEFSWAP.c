...
void copyFromSolutionP1(double*solve,double(*mat)[X+1][N],double (*m)[X+1][N],int j);
void copyFromSolutionDiff(double*solve,double(*mat)[X+1][N],double (*m)[X+1][N],int j);
...
int main(int argc,char *argv[]) {
  ...
  void (*copySolve)(double*,double(*mat)[X+1][N],double (*m)[X+1][N],int j);
...
void copyFromSolutionP1(double*solve,double(*E)[X+1][N],double(*F)[X+1][N],int j) {
  int i = 0,k = 0;
  for ( i = 0; i < NN; i+=2) {
      if (k == X) {
          return;
...
void copyFromSolutionDiff(double*solve,double(*E)[X+1][N],double(*F)[X+1][N],int j) {
    int i = 0;
...
void PredictorCorrectorSolution(int times,int i, void(*f)(),void(*BuildLUD)(),void(*ApplyTS)(),void(*copySolve)()) {
    ...
    (*f)(X+1,N,&EF,E,F,T,i-1);//build EF or FL
...
void constructLUDP1AB(double (*L)[NN],double (*U)[NN],double (*mainD)[NN]) {
    ...
    for (i = 0; i < NN; i++) {
      if (i % 2 == 0) {//E
          ...
          opacity = getOpacity(k,currentTimeStep-1);
          (*mainD)[i] = 1.0 + deltaT*opacity*B[j]/(A[j]);
          //printf("%lf\t",opacity);
          j++;
          k++;
      }
...
void ApplyTandSourceP1(int i,double deltaX,double deltaT) {
    ...
    for ( j = 0; j < X; j++) {
        ...
        if (! (j*deltaX >= x0 || i*deltaT >= t0))
        {
         ...
         solve[2*j] += getOpacity(j,i)*deltaT*T[j][i]+ Src*deltaT;
    }
    //@@@added this
    if (!constOpacity){
      // solve[0] = 0.5*c;
     //solve[1] += deltaT/(2.0*deltaX);
      }
