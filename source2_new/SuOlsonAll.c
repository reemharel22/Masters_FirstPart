...
void PredictorCorrectorSolution(int times,int i, void(*f)(),void(*BuildLUD)(),void(*ApplyTS)(),void(*copySolve)()) {
    ...
    (*BuildLUD)(&L,&U,&mainD); // build LUD
     for (k = 0; k < NN; k++) {
        ...
        si_[k + 1] = U[k];
        r_[k + 1] = solve[k];
        if (a_[k + 1] != a_[k+1] ||b_[k + 1] != b_[k+1] ||si_[k + 1] != si_[k+1] || r_[k + 1] != r_[k+1]  ) {
            printf("Bad matrix\n");
        }
     //   printf("%10e\t%10e\t%10e\t%10e\n", a_[k + 1], b_[k + 1], si_[k + 1],r_[k + 1]);
      ...
      if ( currentTimeStep < 10) {
        for (k = 0; k < 100; k++) {
          //  printf("%10e\t%10e\t%10e\t%10e\t%10e\n",L[k],mainD[k],U[k],solve[k],T[k][currentTimeStep-1]);
        }
       // printf("\n");
    }
   ...
   // alpha = 4.0 * arad;
    ...
    for (i = 0; i < X; i++) {
        for ( j = 0; j < N; j++) {
            ...
            E[i][j] = T[i][j] = pow(tm, 4) *5 * arad; // kelvin
...
void applyBC(int time1) {
  if (constOpacity == 0) {
   ...
   double l = deltaT / (deltaX*deltaX);
    ...
    solve[0] += arad * c * pow(getTH(time1) *5 ,4) * deltaT/(2.0*deltaX);
