...
void constructLUDDiff(double (*L)[NN],double (*U)[NN],double (*mainD)[NN]) {
    ...
    for (i = 0; i < NN; i++) {
        opacity = getOpacity(i,currentTimeStep-1);
        (*mainD)[i] = 1.0 + deltaT*c*opacity + lambda*c*(EF[i+1]+2*EF[i] + EF[i-1])/(2.0);
    }
    //@@@ added 2*deltaX
    double bb = 0;
    if (constOpacity == 0 ) {
...
void ApplyTandSourceDiff(int i,double deltaX,double deltaT) {
    ...
    for ( j = 0; j < X; j++) {
        Src = 0;
        if (! (j*deltaX >= x0 || i*deltaT*c >= t0))
        {
            Src = getSource();
        }
        solve[j] += getOpacity(j,i)*deltaT*c*T[j][i]+ Src*deltaT*c;
    }
    if (!constOpacity) {
