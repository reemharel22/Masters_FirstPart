...
void constructLUDP1(double (*L)[NN],double (*U)[NN],double (*mainD)[NN]) {
    ...
    //@@@@@@@added
    if (!constOpacity) {
  //    (*mainD)[0] += 2.0*c*deltaT/(3.0*deltaX);
        (*U)[0] = 0.5*c;
        (*mainD)[0] = 1.0;
    }
}
    ...
    for ( i = 1; i < NN; i++) {
        if (i % 2 == 1) {//E
            ...
            if (i != NN-1 && i != 0) {
                ...
                mu = mucurrent/muprev;
                (*U)[i] = (deltaT*c*c*mu)/(deltaX*A[j] +1e-15);
                j++;
          }
...
void setUpInitialCondition() {
    ...
    if (P1) {
      ...
      if (!constOpacity) {
       // solve[0] += 4.0*c*getFinc()*deltaT/(3.0*deltaX);
       applyBoundaryCondition();
      }
    }
    else {//diffusion
