...
void buildEFKershaw(double (*EF)[X], double E[X+1][N],double F[X][N],int j);
void buildEta(double (*EF)[X],double E[X+1][N],double F[X][N],int j);
void buildEFLP(double (*EF)[X], double E[X+1][N],double F[X][N],int j);
void buildEFMinerbo(double (*EF)[X], double E[X+1][N],double F[X][N],int j);
void buildNoEF(double (*EF)[X], double E[X+1][N],double F[X][N],int j);
void buildABLambdaT(double (*EF)[X], double E[X+1][N],double F[X][N],int j);
...
double E[X+1][N],T[X][N],matrix[NN][NN];
double L[NN],U[NN],mainD[NN],solve[NN],F[X][N],EF[X],Z[50][500];
double x0 = 0.5;
double t0 = 10.0;
double A[X];
int timestep = 0;
...
void sendToFile(double E[N+1][N],double T[N][N],double deltaX,double deltaT,int p) {
    int i = 0,j;
    FILE*fp;
    if (p == 0) {
...
void ApplyTandSource(int i,double deltaX,double deltaT) {
    ...
    for ( j = 0; j < X; j++) {
        ...
        //opacity here
         solve[2*j] += getOpacity(j,timestep)*deltaT*T[j][i]+ Src*deltaT;
    }
}
